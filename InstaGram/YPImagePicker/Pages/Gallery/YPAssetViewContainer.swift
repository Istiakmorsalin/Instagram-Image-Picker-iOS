...
class YPAssetViewContainer: UIView {
    ...
    override func awakeFromNib() {
        ...
        rotateButton-105-|
        rotateButton.setImage(YPConfig.icons.rotateImage, for: .normal)
    ...
    @objc public func rotateButtonTapped() {
        if let zoomalbeView = zoomableView {
            zoomableView?.photoImageView.transform =  (zoomableView?.photoImageView.transform.rotated(by: CGFloat(M_PI_2)))!
            
            let z = zoomalbeView.zoomScale
            if z >= 1 && z < zoomalbeView.squaredZoomScale {
                shouldCropToSquare = true
            } else {
                shouldCropToSquare = false
            }
